...
- 适配层负责命名空间作用域与前缀遮蔽；库的 `GetAttribute(localName, prefix)` 不代替按 URI 判断 SilkyUI／Binding 命名空间的规则。
- `XmlContextAnalyzer` 继续负责光标处的补全上下文。完整标签使用全部声明；未完成标签只使用光标前可见的声明。
- `sui:*`、`bind:*`、`Body`、`M.*`、C# 元数据、属性绑定和现有语义诊断仍由 SilkyUISupport 负责。普通属性值保留原始文本及其长度；命名空间值沿用已有解码策略。
...
- 输入 `<`、`</`、`<M.`，以及带缺失值或未闭合引号的属性；检查光标处的补全和替换范围。
- 切换命名空间前缀，覆盖祖先声明，在声明尚未完成时请求补全；确认严格的 URI 匹配和前缀遮蔽。
- 在注释、CDATA 和处理指令附近编辑，确认其文本不会被当成普通元素。
- 检查普通元素及逐层嵌套的 `M.Xxx` 成员：只有 getter 或 setter 不公开的对象属性应可继续展开，叶子属性赋值仍要求公开 setter。
- 在嵌套成员中检查不完整 `<M.` 的补全、属性和枚举补全、基类与继承接口成员、只读赋值诊断，以及悬停和开始／结束标签导航。
